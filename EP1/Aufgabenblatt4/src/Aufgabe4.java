...
public class Aufgabe4 {
    ...
    private static void shiftHighestValue(int[] workArray, int index) {
        ...
        int lastIdx = workArray.length - 1;
        if (index == lastIdx)
            return;

        shiftHighestValue(workArray, index + 1);

        if (workArray[index] > workArray[lastIdx]) {
            int temp = workArray[lastIdx];
            workArray[lastIdx] = workArray[index];
    ...
    private static boolean containsValue(int[] workArray, int value) {
        ...
        int med = workArray.length / 2;
        int[] left = Arrays.copyOfRange(workArray, 0, med);
        int[] right = Arrays.copyOfRange(workArray, med, workArray.length);
